...
def test_I009_person_lifecycle_closure_and_irreversibility(person_repo, person_repo_factory):
    """
    ...
    assert person_repo.get_by_id("person-1") == person

    # read/update allowed
    updated = Person(id="person-1", name="Alicia", email="a@example.com")
    person_repo.update(updated)
    assert person_repo.get_by_id("person-1") == updated
