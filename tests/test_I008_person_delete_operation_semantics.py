...
def test_I008_person_delete_operation_semantics():
    """
    ...
    repo.delete("person-1")

    # Deleted entity must no longer be observable
    assert repo.get_by_id("person-1") is None
    ...
    repo.create(p2)
    assert repo.get_by_id("person-1") == p2
    assert repo.get_by_id("person-1").name == "Bob"
